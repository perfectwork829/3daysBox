...
export class MembershipComponent implements OnInit, AfterViewInit{
  ...
  ngOnInit() {    
    this.title.setTitle(this.coreTranslationService.translateText('Membership - 3 Days'));
		this.meta.addTags([{
        property: 'og:title',
        content: this.coreTranslationService.translateText('Membership - 3 Days'),
      },
    ]);
  ...
  reactivateSubscription(subs){      
    ...
    personalConfirm.then((result) => {
      if (result.isConfirmed) {
        ...
        this.loaderService.setLoading(true);   
        console.log('subscriptioin id', subs.id);
