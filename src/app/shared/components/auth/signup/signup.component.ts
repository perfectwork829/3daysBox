...
export class SignupComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    if(this.user.gender!='all'){
      this.onGenderSelect(this.user.gender);
    }    
  }
  ...
  setDefaultValues(){
    console.log('default branch infore here', this.config.user);
  ...
  onGenderSelect(type: string){
    console.log('gender selection', type)
  ...
  onFetchBranches(){    
    ...
    console.log('branch2', this.config.user.gender);
    console.log('branch3', this.signup.gender);
    ...
    this.httpService.post('branches', {keyword: '', client_id: this.httpService.client_id, gender: this.config.token? this.config.user.gender:this.signup.gender}).then((response: any)=>{
      ...
      if(this.selectedProduct){
        //this.branches = response.filter(item=> item.id==7);
  ...
  onCheckCoupon(){
    this.httpService.post('check-coupon', { coupon_code: this.signup.promotional_code, product_id: this.product.id, member_id:this.config.user.id }).then((response: any)=>{
      ...
      this.checkoutInfo.coupon_code = response.coupon_code;
  ...
  onSelectProduct(product: Product){
    ...
    //this.checkoutRedirectUrl = 'membership';
  ...
  //Open the signup's terms and conditions dialog.
