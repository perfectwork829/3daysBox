...
export class CheckOutComponent  implements OnInit, AfterViewInit {
  ...
  onCheckCoupon(){
    this.httpService.post('check-coupon', { coupon_code: this.form.promotional_code, product_id: this.product.id, member_id:this.config.user.id })
      .then((response: any)=>{
        ...
        console.log('onCheckCoupon checkoutInfo', this.checkoutInfo);
    }).catch(response => {
      console.log('onCheckCoupon', response);
  ...
  showGosell(){
    ...
    goSell.config({
      ...
      callback:(response) => {          
        if (response?.id){
          console.log('successfuly pament===========', response);
  ...
  onPayNow(data:any){
    console.log('pay validation start~~ checkout page environment test key here', this.checkoutInfo);
    ...
    this.httpService.post('member/subscriptions/buy/validate', this.checkoutInfo).then((response: any)=>{
      ...
      if(!response){
        ...
        console.log('ember/subscriptions/buy/valid', this.checkoutInfo);
